...
public:
	...
	void wait(unsigned long millis) override;
...
inline bool CoroutineImpl::update(unsigned long millis) {
	if (suspended)
		return false;
...
inline void CoroutineImpl::reset() {
	barrierTime = 0;
	sinceStarted = 0;
	jumpLocation = 0;
...
inline void CoroutineImpl::wait(unsigned long time) {
	barrierTime = millis() + time;
}

inline void CoroutineImpl::freeLocals() {
	if (numSavedLocals == 0)
		return;
...
inline void CoroutineImpl::terminate() {
	terminated = true;
	suspended = false;
	looping = false;
	jumpLocation = -1;
...
inline void CoroutineImpl::suspend() {
...
inline void CoroutineImpl::resume() {
	if (suspended && !terminated) {
		suspended = false;
...
inline void CoroutineImpl::loop() {
	jumpLocation = 0;
	numRecoveredLocals = 0;
	looping = true;
	trace(P("...looping..."));
...
inline bool CoroutineImpl::isTerminated() const {
	return terminated;
}

inline bool CoroutineImpl::isSuspended() const {
	return suspended;
}
