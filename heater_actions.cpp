...
update_heater_status::update_heater_status() {
...
/*And here's my execute! It turns out this action is listening 
for an event named " avg_temp_update " that is triggered every 5 
seconds. You can see that code in arduino_mega.cpp if you'd like!

First, we turn the "args" data into temperature data so we can 
access it.

Then, we have a bit of logic... If the average temp is less than 
0 degrees celcius, turn the heater on to 20% power. Otherwise, if 
the temperature is greater than 5 degrees celcius, turn the heater 
off.*/
